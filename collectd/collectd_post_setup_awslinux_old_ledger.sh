#!/usr/bin/env bash
# collectd_post_setup_awslinux_old_ledger.sh
#
...
# be executed as root on ledger servers running old versions
# of Amazon Linux < v2 using upstart init (not systemd).
...
echo -e "\n"  # add newline at EOF
echo 'df-mnt-log-percent_bytes-used' >> $whitelist
echo 'processes.*' >> $whitelist
