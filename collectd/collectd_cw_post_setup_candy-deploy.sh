#!/usr/bin/env bash
# collectd_cw_post_setup_candy-deploy.sh
#
...
# be executed as root on "jenkins" running Ubuntu 16.04
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "candy-deploy" "candy-sb-admin.jar"
        ProcessMatch "nginx" "/usr/sbin/nginx"
        ProcessMatch "candy-deploy" "jenkins.war"
