#!/usr/bin/env bash
# collectd_cw_post_setup_gl-batch-1.sh
#
...
# be executed as root on "gl-batch-1" server running Ubuntu 16.04+
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "venomsolo" "-jar venom-solo.jar"
        ProcessMatch "spidersolo" "-jar spider-solo.jar"
