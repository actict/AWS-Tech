#!/usr/bin/env bash
# collectd_cw_post_setup_genesis.sh
#
...
# be executed as root on "genesis" intranet server running
# Ubuntu 18.04 in Actwo office.
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "wg-peer" "wg-crypt-peerne"
        ProcessMatch "wg-devnet" "wg-crypt-devnet"
        ProcessMatch "wg-gdac" "wg-crypt-gdac"
        ProcessMatch "wg-gate" "wg-crypt-gate"
        ProcessMatch "dnsmasq" "/usr/sbin/dnsmasq"
