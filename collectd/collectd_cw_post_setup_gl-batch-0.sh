#!/usr/bin/env bash
# collectd_cw_post_setup_gl-batch-0.sh
#
...
# be executed as root on "gl-batch-0" server running Ubuntu 16.04+
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "venom" "-jar venom.jar"
        ProcessMatch "spider" "-jar spider.jar"
