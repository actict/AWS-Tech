#!/usr/bin/env bash
# collectd_cw_post_setup_ubuntu_etc.sh
#
...
# "ETC_prod" (Ethereum Classic Server) running Ubuntu 18.04 with
# systemd (not upstart).
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "geth" "/usr/local/bin/geth"
</Plugin>
EOF
...
df-home-ethereumproject-.ethereum-classic-percent_bytes-used
processes.*
EOF
