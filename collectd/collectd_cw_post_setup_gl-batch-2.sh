#!/usr/bin/env bash
# collectd_cw_post_setup_gl-batch-2.sh
#
...
# be executed as root on "gl-batch-2" server running Ubuntu 16.04+
...
cat >> "$coldconf" <<EOF
<Plugin processes>
        ProcessMatch "venommining" "-jar venom-mining.jar"
        ProcessMatch "spidermining" "-jar spider-mining.jar"
