...
log_grp="odr-svr_logs"
filterp='[loglevel=ERROR, ID, timestamp, msg]'
t_start=$(date -u +%F' '%H:%M:%S --date '-130 sec')
t_end=$(date -u +%F' '%H:%M:%S --date '-60 sec')
resultf="$appfolder/order-svr-loglevel_error.log"
logfile="$appfolder/get_cw_logs_order-svr_error.log"
