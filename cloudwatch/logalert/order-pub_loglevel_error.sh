...
log_grp="odr-pub_logs"
filterp='[loglevel=ERROR, ID, timestamp, msg]'
t_start=$(date -u +%F' '%H:%M:%S --date '-130 sec')
t_end=$(date -u +%F' '%H:%M:%S --date '-60 sec')
resultf="$appfolder/order-pub-loglevel_error.log"
logfile="$appfolder/get_cw_logs_order-pub_error.log"
