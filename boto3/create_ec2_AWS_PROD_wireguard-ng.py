#!/usr/bin/env python3
# create_ec2_AWS_PROD_wireguard-ng.py
#
# Last Updated: 2019.10.31
# Updated by: jun.go@peertec.com
#
# This script uses boto3 to create an EC2 instance that will be used
# as a Wireguard VPN 'new generation' (ng) server for AWS PROD 'gdac'
# VPC. This server will not use NAT; instead wireguard iface IP's and
# Peertec office internal network IP's will be added to AWS VPC routing
# tables to enable comms to VPC IP range 10.0.0.0/16
...
def main():
    ...
    my_inst_name = 'gdac-prod-exchange-wireguard-ng'
    ...
    instance = ec2Resource.create_instances(
        ...
        NetworkInterfaces = [
            {
                'DeviceIndex': 0,
                'SubnetId': subnet_dict['dmz-mgmt-2a'],
                'Groups': [
                    sec_group_dict['wireguard-ng'],
                ],
                'AssociatePublicIpAddress': False
            }
        ],
        KeyName = ssh_keys_dict['wireguard-prod-ng'],
        DryRun = False
    )
