#!/usr/bin/env python3
# create_all_cloudwatch_alarms_for_candy_pecun.py
#
# Last Updated: 2019.07.03
# Updated by: scott.hwang@peertec.com
#
# This script will create CPU, Memory, root disk, and process alarms
# for all EC2 'candy*' servers running in AWS PECUN Seoul region.
...
from env_pecunian import sns_topic_dict, instance_id_dict
...
def main():
    ...
    dictOfInst = {
        ...
        'crawler' : [sns_topic_dict['crawler'],
                    instance_id_dict['crawler'], '',
                     ['candy-crawler', 'cloudwatch', 'nginx']],
        'crawler-orderbook' : [sns_topic_dict['crawler-odrbk'],
                               instance_id_dict['crawler-orderbook'], '',
                               ['candy-crawler-orderbook', 'cloudwatch']],
        'crawler-price' : [sns_topic_dict['crawler-price'],
                           instance_id_dict['crawler-price'], '',
                           ['candy-crawler-price', 'cloudwatch']],
        'db-redis' : [sns_topic_dict['db-redis'],
                      instance_id_dict['db-redis'], '',
                      ['candy-db-redis', 'cloudwatch', 'nginx']],
        'candy-deploy' : [sns_topic_dict['candy-deploy'],
                          instance_id_dict['deploy'], '',
                          ['candy-deploy', 'cloudwatch', 'nginx']],
        'halls' : [sns_topic_dict['halls'], instance_id_dict['halls'],
                   '', ['halls', 'cloudwatch', 'nginx']],
        'halls-cow' : [sns_topic_dict['halls-cow'],
                       instance_id_dict['halls-cow'], '',
                       ['halls-cow', 'cloudwatch']],
        'ricola' : [sns_topic_dict['ricola'],
                       instance_id_dict['ricola'], '',
                       ['ricola', 'cloudwatch', 'nginx']],
    }

    session = boto3.Session(profile_name = 'pecun_cw_full')
