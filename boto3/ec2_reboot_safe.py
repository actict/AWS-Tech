...
# instance ID's from the command line and will reboot each
# of the instances. Before rebooting, the user will be
# presented with the instance's tag 'Name:' and will have to
# type 'Yes' to confirm the reboot operation.
...
def rebootInstance(eco, iid):
    ...
    """
    resp = eco.reboot_instances(InstanceIds = [iid])
    return resp
...
def main():
    ...
    for id in args.iid:
        ...
        while confirm not in ('Yes', 'no'):
            confirm = input("Do you really want to reboot InstanceID "
                            "%s with Name %s?\n"
                            "Please type 'Yes' or 'no':\n"
                            %(id, inst_name))
        if confirm == 'Yes':
            print("## %d. Rebooting InstanceID %s with Name %s ##"
                  %(inst_cnt, id, inst_name))
            reboot_status = rebootInstance(ec2Client, id)
            http_stat = reboot_status['ResponseMetadata']['HTTPStatusCode']
            if http_stat == 200:
