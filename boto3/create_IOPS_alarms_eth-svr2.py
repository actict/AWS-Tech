#!/usr/bin/env python3
# create_IOPS_alarms_eth-svr2.py
#
# Last Updated: 2018.11.16
# Updated by: hulk.choi@actwo.com
#
# This script creates cloudwatch alarms for an io1 IO-optimized
# disk storing the Ethereum blockchain. This disk has a capacity
# of 1000GB (1TB) and has 5000 IOPS reserved.


import boto3


volid = 'vol-0d77c756315f5ab6c'
sns_topic_wallet = 'arn:aws:sns:ap-northeast-2:905136931838:walletNoti'

session = boto3.Session(profile_name = 'dev')
cwatchClient = session.client('cloudwatch')
readIOPS_max = 2700
readops_max = readIOPS_max * 60
writeIOPS_max = 2300
writeops_max = writeIOPS_max * 60

print("Creating eth-svr2 read IOPS alarm")
resp = cwatchClient.put_metric_alarm(
    AlarmName = 'eth-svr2-read-IOPS-gt-2700',
    AlarmDescription = 'Alarm when Read I/O Ops gt 162,000 in 1 min',
    ComparisonOperator = 'GreaterThanThreshold',
    EvaluationPeriods = 5,
    MetricName = 'VolumeReadOps',
    Namespace = 'AWS/EBS',
    Period = 60,
    Statistic = 'Average',
    Threshold = readops_max,
    AlarmActions = [
        sns_topic_wallet
    ],
    Dimensions = [
        {
          'Name': 'VolumeId',
          'Value': volid
        },
    ],
    Unit = 'Count')

if resp['ResponseMetadata']['HTTPStatusCode'] == 200:
    print("Alarm creation SUCCESS")
else:
    print("Alarm creation ERROR")
    print(resp)

print("Creating eth-svr2 write IOPS alarm")
resp = cwatchClient.put_metric_alarm(
    AlarmName = 'eth-svr2-write-IOPS-gt-2300',
    AlarmDescription = 'Alarm when Write I/O Ops gt 138,000 in 1 min',
    ComparisonOperator = 'GreaterThanThreshold',
    EvaluationPeriods = 5,
    MetricName = 'VolumeReadOps',
    Namespace = 'AWS/EBS',
    Period = 60,
    Statistic = 'Average',
    Threshold = writeops_max,
    AlarmActions = [
        sns_topic_wallet
    ],
    Dimensions = [
        {
          'Name': 'VolumeId',
          'Value': volid
        },
    ],
    Unit = 'Count')

if resp['ResponseMetadata']['HTTPStatusCode'] == 200:
    print("Alarm creation SUCCESS")
else:
    print("Alarm creation ERROR")
    print(resp)
