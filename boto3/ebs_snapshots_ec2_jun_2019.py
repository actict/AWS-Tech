...
from env_pecunian import ebs_vol_dict as vol_dict_pec


session0 = boto3.Session(profile_name = 'prod_ec2_full')
session1 = boto3.Session(profile_name = 'pec_ec2_full')
ec2Client0 = session0.client('ec2')
ec2Client1 = session1.client('ec2')
...
for vol in vol_dict_pec:
    if vol in snap_vols_pecL:
        print("Taking snapshot of : %s disk" % vol)
        resp = ec2Client1.create_snapshot(
            VolumeId = vol_dict_pec[vol],
            TagSpecifications = [
                {
                    'ResourceType': 'snapshot',
                    'Tags': [
