#!/usr/bin/env python3
# create_ec2_candy_admin_pecun.py
#
# Last Updated: 2019.07.26
# Updated by: scott.hwang@peertec.com
#
# This script uses boto3 to create an EC2 instance that will be used
# for candy-admin in AWS PECUN. Note that for Ubuntu 18.04 AMI using gp2 ssd,
# the root disk will always use /dev/sda1 device name while non-root
# disks use /dev/xvdY format.
...
def main():
    start = time.time()
    session = boto3.Session(profile_name = 'pec_ec2_full')
    client = session.client('ec2')
    ec2Resource = session.resource('ec2')
    # basename to give to new EC2's
    my_inst_name = 'pec-prod-candy-admin'
    ...
    instance = ec2Resource.create_instances(
        ...
        NetworkInterfaces = [
            {
                'DeviceIndex': 0,
                'SubnetId': subnet_dict['public-2a'],
                'Groups': [
                    sec_group_dict[''],
        ...
        KeyName = ssh_keys_dict['candy-seoul'],
        DryRun = False
    )
