#!/usr/bin/env python3
# get_cloudwatch_logs_RDS_except_alrime.py
#
...
#
# Note: this script has been modified to exclude logs that contain the
# string 'alrime' and a proper timestamp of the form "# Time: yymmdd"
# in query results
...
def main():
    ...
    # only look at logs that don't contain the string 'alrime'
    for i in range(len(payload)):
        if "alrime" not in payload[i]['message']:
            # only include logs with a timestamp
            if "# Time" in payload[i]['message']:
                noAlrimeL.append(payload[i]['message'])

    if len(noAlrimeL) > 0:
        with open(args.filename, 'w') as f:
            print()
            f.write('\n')
            for j in range(len(noAlrimeL)):
                print(noAlrimeL[j])
                f.write(noAlrimeL[j])
                print()
                f.write('\n')
