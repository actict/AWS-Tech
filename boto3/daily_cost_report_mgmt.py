#!/usr/bin/env python3
# daily_cost_report_mgmt.py
#
# Last Updated: 2019.12.19
# Updated by: scott.hwnag@peertec.com
#
...
def prez_extract_ce(res_dict, rpt_date, outfile):
    ...
    *cost by service* (coarse) and *cost by utilization type* (detailed).
    ...
    ec2_compute_t3_xlarge = Decimal(res_dict[
        'Amazon Elastic Compute Cloud - Compute'].get(
            'RunInstances: APN2-BoxUsage:t3.xlarge', 0))
    ...
    total_cost_svc = (ec2_compute + ec2_other)

    with open(outfile, 'w') as f:
        print("*TOTAL COST in MGMT* on %s:\t*$%s*"
              %(rpt_date, total_cost_svc.quantize(Decimal("1.00"))))
        f.write("*TOTAL COST in MGMT* on %s:\t*$%s*\n"
                %(rpt_date, total_cost_svc.quantize(Decimal("1.00"))))
        print("==================================================")
        f.write("==================================================\n")
        print("\t`EC2-compute`:\t$%s"
...
def main():
    ...
    ce_general = file_path + 'cost_explorer_daily_mgmt_' + Tend + '.txt'
    # boto3 session vars
    session = boto3.Session(profile_name = 'mgmt_ce_full_ec2_ro')
    ...
    if args.mode == 'real':
        print("Send non-RI Daily MGMT Cost Explorer data to Slack")
        http_status = send_to_slack(ce_general, slack_webhook,
                                    'AWS_Cost_Explorer MGMT')
        if http_status != 200:
            print("Sending MGMT AWS Cost Explorer data payload to %s FAILED!"
                  % slack_webhook)
        else:
            print("Sending MGMT AWS Cost Explorer data payload to %s SUCCESS!"
                  % slack_webhook)
