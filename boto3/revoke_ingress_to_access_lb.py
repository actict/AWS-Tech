...
'''
revoke_ingress_to_access_lb.py

Last Updated: 2019.10.21
Updated by: joshua.huh@actwo.com, scott.hwang@peertec.com

Purpose: revoke ALL ingress traffic to ALB for Elastic Beanstalk
app environment 'prod-access' and set LB idle timeout to 1s to
cut off connections in flight.

Revoking ingress traffic in LB is NOT enough!!!
Even minimum idle timeout is too log to clear all sessions.
Temporary traffic choking in the server is required via Security
Groups.
'''
...
print("### Results of changing prod-access LB idle timeout to 1 sec ###")
print(resp)
...
print("### Results of revoking prod-access LB SG ingress ###")
print(resp1)

time.sleep(1)

resp2 = ec2Client.revoke_security_group_ingress(
    GroupId = sec_group_dict['access'],
    IpPermissions = [
        {
            'FromPort': 80,
            'IpProtocol': 'tcp',
...
print("### Results of revoking ingress SG to prod-access server ###")
print(resp2)
...
print("### Results of authorizing ingress SG to prod-access server ###")
print(resp3)
